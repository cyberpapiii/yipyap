...
-- PRODUCTION VERSION: Handles unmapped combinations gracefully
...
BEGIN
  ...
  RAISE NOTICE 'PRODUCTION MODE: Proceeding with migration despite unmapped combinations.';
  RAISE NOTICE 'Unmapped combinations will use default fallback values (A line, mta-blue).';
END;
$$;

-- Show the unmapped combinations for logging
DO $$
DECLARE
  rec RECORD;
BEGIN
  RAISE NOTICE 'Unmapped emoji/color combinations found:';
  FOR rec IN
    SELECT emoji, color, COUNT(*) as user_count
    FROM users
    WHERE (emoji, color) NOT IN (
  ...
  LOOP
    RAISE NOTICE '  % + % = % users (will map to A + mta-blue)', rec.emoji, rec.color, rec.user_count;
  END LOOP;
END;
$$;
